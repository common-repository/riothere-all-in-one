...
class Riothere_All_In_One_Free_Shipping_Promotions
{
    ...
    public function __construct($plugin_name, $version)
    {
        ...
        add_action('init', [$this, 'riothere_register_free_shipping_promotions_custom_post_type']);
        add_action('add_meta_boxes', [$this, 'riothere_add_free_shipping_promotions_settings_fields']);

        // Handle Saving/Adding promotion. Triggered when the post type get updated
        add_action('save_post', [$this, 'riothere_handle_save_free_shipping_promotions']);

        // ajax call for the admin UI
        add_action('wp_ajax_free_shipping_promotions_get_products', array($this, 'riothere_free_shipping_ajax_get_products'));
        add_action('wp_ajax_free_shipping_promotions_get_products_by_rules', array(
            $this,
            'riothere_free_shipping_ajax_promotions_get_products_by_rules',
        ));
         ...
         * */
        add_action('before_delete_post', [$this, 'riothere_free_shipping_handle_deleting_product']);
        add_action('save_post', array($this, 'riothere_free_shipping_handle_save_product'), 1000);
        add_action('woocommerce_product_import_inserted_product_object', array(
            $this,
            'riothere_free_shipping_handle_save_product_from_import',
        ));
    }

    public function riothere_free_shipping_handle_save_product_from_import($product)
    {
        $this->riothere_free_shipping_handle_save_product($product->get_id());
    }

    public function riothere_free_shipping_handle_save_product($product_id)
    {
        if (get_post_type($product_id) === 'product') {
            $free_shipping_promotions = get_posts([
                'post_type' => 'free-shipping-promotion',
                'status' => 'publish',
                'nopaging' => true,
            ]);
            ...
            foreach ($free_shipping_promotions as $promotion) {
                $free_shipping_promotion_id = $promotion->ID;
                $give_promotion_to_new_added_items = get_post_meta($free_shipping_promotion_id, 'give_promotion_to_new_added_items', true);
                $new_applicable_product_ids = self::get_new_applicable_product_ids($free_shipping_promotion_id);
                $sellers_excluded_product_ids = self::get_sellers_excluded_product_ids($free_shipping_promotion_id);
                $previously_applicable_product_ids = self::get_applicable_product_ids($free_shipping_promotion_id);
                ...
                if (!in_array($product_id, $sellers_excluded_product_ids)) {
                    ...
                    if (in_array($product_id, $previously_applicable_product_ids) && !in_array($product_id, $new_applicable_product_ids)) {
                        update_post_meta($free_shipping_promotion_id, 'promotion_applicable_product_ids', array_diff($previously_applicable_product_ids, [$product_id]));
                        update_post_meta($product_id, 'free_shipping_promotion_ids', array_diff($product_promotion_ids, [$free_shipping_promotion_id]));
                        ...
                        if (in_array($product_id, $new_applicable_product_ids) && !in_array($product_id, $previously_applicable_product_ids)) {
                            update_post_meta($free_shipping_promotion_id, 'promotion_applicable_product_ids', array_unique(array_merge($previously_applicable_product_ids, [$product_id])));
                            if ($seller_id !== null) {
                                // @todo phase 3 send emails you have the seller id and the $product_ids newly added into promotion ($seller_id, [$product_id] )
                                update_post_meta($product_id, 'free_shipping_promotion_ids', array_unique(array_merge($product_promotion_ids, [$free_shipping_promotion_id])));
                            }
                        }
                    }
    ...
    public function riothere_free_shipping_handle_deleting_product($product_id)
    {
        if (get_post_type($product_id) === 'product') {
            ...
            foreach ($promotions as $promotion) {
                ...
                update_post_meta($free_shipping_promotion_id, 'promotion_applicable_product_ids', $applicable_product_ids);
            }
    ...
    {
        $free_shipping_promotion_ids = get_post_meta($product_id, 'free_shipping_promotion_ids', true);
        if ($free_shipping_promotion_ids === '') {
            $free_shipping_promotion_ids = [];
        }

        return $free_shipping_promotion_ids;
    }
    ...
    public function riothere_enqueue_scripts()
    {
        ...
        wp_enqueue_script(
            $this->plugin_name . '_free_shipping_promotions',
            plugin_dir_url(__FILE__) . 'js/riothere-all-in-one-free-shipping-promotions.js',
        ...
        wp_localize_script(
            $this->plugin_name . '_free_shipping_promotions',
            'riothere_admin_free_shipping_promotions_global',
            array(
                'ajax_url' => admin_url('admin-ajax.php'),
                'free_shipping_promotions_nonce' => wp_create_nonce('free_shipping_promotions'),
            )
        );
    }

    public function riothere_register_free_shipping_promotions_custom_post_type()
    {
        $args = array(

            'label' => __('Free Shipping Promotion', 'riothere-all-in-one'),
            'description' => __('', 'riothere-all-in-one'),
            'labels' => array(
                'name' => _x('Free Shipping Promotions', 'Post Type General Name', 'riothere-all-in-one'),
                'singular_name' => _x('Free Shipping', 'Post Type Singular Name', 'riothere-all-in-one'),
                'menu_name' => _x('Free Shipping Promotions', 'Admin Menu text', 'riothere-all-in-one'),
                'name_admin_bar' => _x('Free Shipping Promotion', 'Add New on Toolbar', 'riothere-all-in-one'),
                'archives' => __('Free Shipping Promotion Archives', 'riothere-all-in-one'),
                'attributes' => __('Free Shipping Promotion Attributes', 'riothere-all-in-one'),
                'parent_item_colon' => __('Parent Free Shipping Promotion:', 'riothere-all-in-one'),
                'all_items' => __('Free Shipping Promotions', 'riothere-all-in-one'),
                'add_new_item' => __('Add New Free Shipping Promotion', 'riothere-all-in-one'),
                'add_new' => __('Add New', 'riothere-all-in-one'),
                'new_item' => __('New Free Shipping Promotion', 'riothere-all-in-one'),
                'edit_item' => __('Edit Free Shipping Promotion', 'riothere-all-in-one'),
                'update_item' => __('Update Free Shipping Promotion', 'riothere-all-in-one'),
                'view_item' => __('View Free Shipping Promotion', 'riothere-all-in-one'),
                'view_items' => __('View Free Shipping Promotions', 'riothere-all-in-one'),
                'search_items' => __('Search Free Shipping Promotion', 'riothere-all-in-one'),
                ...
                'insert_into_item' => __('Insert into Free Shipping Promotion', 'riothere-all-in-one'),
                'uploaded_to_this_item' => __('Uploaded to this Free Shipping Promotion', 'riothere-all-in-one'),
                'items_list' => __('Free Shipping Promotions list', 'riothere-all-in-one'),
                'items_list_navigation' => __('Free Shipping Promotions list navigation', 'riothere-all-in-one'),
                'filter_items_list' => __('Filter Free Shipping Promotions list', 'riothere-all-in-one'),
            ),
            'menu_icon' => '',
        ...
        register_post_type('free-shipping', $args);
    }

    public function riothere_add_free_shipping_promotions_settings_fields()
    {
        add_meta_box(
            'free_shipping_promotions_settings', // Unique ID
            esc_html__('Free Shipping Promotions', 'example'), // Title
            array($this, 'riothere_free_shipping_promotion_meta_box_display'), // Callback function
            'free-shipping', // Admin page (or post type)
            'advanced', // Context
            'default' // Priority
        );
    }

    public function riothere_free_shipping_promotion_meta_box_display()
    {
        require_once __DIR__ . '/partials/riothere-all-in-one-free-shipping-promotion-display.php';

    }

    private function save_free_shipping_promotion($free_shipping_promotion_id)
    {
        ...
        if ($is_add_mode || $edit_promotion_settings) {
            if (isset($cart_price_greater_than)) {
                update_post_meta($free_shipping_promotion_id, 'cart_price_greater_than', $cart_price_greater_than);
            }

            if (isset($cart_price_less_than)) {
                update_post_meta($free_shipping_promotion_id, 'cart_price_less_than', $cart_price_less_than);
            }

            // @todo validate that its a valid date
            if (isset($promotion_start_date) && !empty($promotion_start_date)) {
                update_post_meta($free_shipping_promotion_id, 'promotion_start_date', $promotion_start_date);
            } else {
                delete_post_meta($free_shipping_promotion_id, 'promotion_start_date');
            }

            // @todo validate that its a valid date
            if (isset($promotion_end_date) && !empty($promotion_end_date)) {
                update_post_meta($free_shipping_promotion_id, 'promotion_end_date', $promotion_end_date);
            } else {
                delete_post_meta($free_shipping_promotion_id, 'promotion_end_date');
            }

            if (isset($selected_countries)) {
                update_post_meta($free_shipping_promotion_id, 'selected_countries', $selected_countries);
            } else {
                delete_post_meta($free_shipping_promotion_id, 'selected_countries');
            }

            if (isset($selected_customers)) {
                update_post_meta($free_shipping_promotion_id, 'selected_customers', $selected_customers);
            } else {
                delete_post_meta($free_shipping_promotion_id, 'selected_customers');
            }
        ...
        if ($is_add_mode || $edit_promotion_rules) {
            ...
            if (is_array($exclude_products) && count($exclude_products) > 0) {
                update_post_meta($free_shipping_promotion_id, 'exclude_products', $exclude_products);
            } else {
                delete_post_meta($free_shipping_promotion_id, 'exclude_products');
            }

            if (is_array($exclude_sellers)) {
                update_post_meta($free_shipping_promotion_id, 'exclude_sellers', $exclude_sellers);
            } else {
                delete_post_meta($free_shipping_promotion_id, 'exclude_sellers');
            }

            if (is_array($exclude_product_categories)) {
                update_post_meta($free_shipping_promotion_id, 'exclude_product_categories', $exclude_product_categories);
            } else {
                delete_post_meta($free_shipping_promotion_id, 'exclude_product_categories');
            }

            if (is_array($exclude_product_tags)) {
                update_post_meta($free_shipping_promotion_id, 'exclude_product_tags', $exclude_product_tags);
            } else {
                delete_post_meta($free_shipping_promotion_id, 'exclude_product_tags');
            }

            if (isset($exclude_product_min_price) && !is_nan((float) $exclude_product_min_price) && (float) $exclude_product_min_price >= 0) {
                update_post_meta($free_shipping_promotion_id, 'exclude_product_min_price', $exclude_product_min_price);
            } else {
                delete_post_meta($free_shipping_promotion_id, 'exclude_product_min_price');
            }

            if (isset($exclude_product_max_price) && !is_nan((float) $exclude_product_max_price) && (float) $exclude_product_max_price >= 0) {
                update_post_meta($free_shipping_promotion_id, 'exclude_product_max_price', $exclude_product_max_price);
            } else {
                delete_post_meta($free_shipping_promotion_id, 'exclude_product_max_price');
            }

            if (isset($give_promotion_to_new_added_items) && !empty($give_promotion_to_new_added_items)) {
                update_post_meta($free_shipping_promotion_id, 'give_promotion_to_new_added_items', $give_promotion_to_new_added_items);
            } else {
                delete_post_meta($free_shipping_promotion_id, 'give_promotion_to_new_added_items');
            }
            ...
            if (isset($filter_exclude_products_by_filter) && !empty($filter_exclude_products_by_filter)) {
                update_post_meta($free_shipping_promotion_id, 'filter_exclude_products_by_filter', $filter_exclude_products_by_filter);
            } else {
                delete_post_meta($free_shipping_promotion_id, 'filter_exclude_products_by_filter');
            }
        }

    }

    public function riothere_handle_save_free_shipping_promotions($free_shipping_promotion_id)
    {
        if (!isset($_POST['free_shipping_promotions_settings_nonce'])) {
            return $free_shipping_promotion_id;
        }
        $nonce = $_POST['free_shipping_promotions_settings_nonce'];

        if (!wp_verify_nonce($nonce, 'free_shipping_promotions_settings_data')) {
            return $free_shipping_promotion_id;
        }
        ...
        $previously_applicable_product_ids = self::get_applicable_product_ids($free_shipping_promotion_id);
        $this->save_free_shipping_promotion($free_shipping_promotion_id);
        ...
        foreach ($new_applicable_product_ids as $applicable_product_id) {
            ...
            if (!in_array($applicable_product_id, $product_promotion_ids)) {
                $product_promotion_ids[] = $free_shipping_promotion_id;
                update_post_meta($applicable_product_id, 'free_shipping_promotion_ids', array_unique($product_promotion_ids));
            }
        }
        ...
        foreach ($previously_applicable_product_ids as $previous_added_product_id) {
            if (!in_array($previous_added_product_id, $new_applicable_product_ids)) {
                ...
                if (count($product_promotion_ids) > 0 && in_array($previous_added_product_id, $product_promotion_ids)) {
                    $updated_product_promotion_ids = array_diff($product_promotion_ids, [$free_shipping_promotion_id]);
                    update_post_meta($previous_added_product_id, 'free_shipping_promotion_ids', array_unique($updated_product_promotion_ids));
                }
            }
        }

        update_post_meta($free_shipping_promotion_id, 'promotion_applicable_product_ids', $new_applicable_product_ids);
    ...
    public function riothere_free_shipping_ajax_promotions_get_products_by_rules()
    {
        check_ajax_referer('free_shipping_promotions', 'ajax_nonce');
        $result = [
            'success' => true,
            'data' => [],
        ];
    ...
    public static function get_sellers_excluded_product_ids($free_shipping_promotion_id)
    {
        $sellers_excluded_product_ids = get_post_meta($free_shipping_promotion_id, 'sellers_excluded_product_ids', true);
    ...
    public static function get_excluded_product_ids($free_shipping_promotion_id)
    {
        $excluded_product_ids = get_post_meta($free_shipping_promotion_id, 'exclude_products', true);
    ...
    public static function get_applicable_product_ids($free_shipping_promotion_id)
    {
        $promotion_applicable_product_ids = get_post_meta($free_shipping_promotion_id, 'promotion_applicable_product_ids', true);
    ...
    public static function get_product_ids_without_sellers_excluded($free_shipping_promotion_id)
    {
        $promotion_applicable_product_ids = get_post_meta($free_shipping_promotion_id, 'promotion_applicable_product_ids', true);
        if ($promotion_applicable_product_ids === '') {
            return [];
     ...
     * */
    public static function get_new_applicable_product_ids($free_shipping_promotion_id)
    {
        $include_rules = self::get_promotion_products_rules_by_type($free_shipping_promotion_id, 'include');
        $exclude_rules = self::get_promotion_products_rules_by_type($free_shipping_promotion_id, 'exclude');
    ...
    public static function get_promotion_products_rules_by_type($free_shipping_promotion_id, $type)
    {
        $rules = [];

        $products = get_post_meta($free_shipping_promotion_id, $type . '_products', true);
        $sellers = get_post_meta($free_shipping_promotion_id, $type . '_sellers', true);
        $product_categories = get_post_meta($free_shipping_promotion_id, $type . '_product_categories', true);
        $product_tags = get_post_meta($free_shipping_promotion_id, $type . '_product_tags', true);
        $product_min_price = get_post_meta($free_shipping_promotion_id, $type . '_product_min_price', true);
        $product_max_price = get_post_meta($free_shipping_promotion_id, $type . '_product_max_price', true);
        if ($products === '') {
            $products = [];
        }
        if ($sellers === '') {
    ...
    public function riothere_free_shipping_ajax_get_products()
    {
        check_ajax_referer('free_shipping_promotions', 'ajax_nonce');
